...
def calculate_num_keywords(text):
    ...
    return max(min(int(num_words / 1000), 20), 5)
            ...
            if file.endswith('.txt'):
                ...
                try:
                    with open(file_path, 'r', encoding='utf-8') as f:
                        ...
                        # 这里计算关键词数量的方法有问题，输入的是整个文本的中字符数量，而不是单词数量。进行修改。
                        ...
                        keywords_result = keywords(tokenized_text, split=True)
...
# gensim 库的 keywords 函数主要用于提取英文文本中的关键词，它使用 TF-IDF（Term Frequency-Inverse Document Frequency）算法来计算关键词权重。对于中文文本，由于中文的语言特点，需要进行分词处理。
