...
for root, dirs, files in os.walk(folder_path):
    for file in files:
        if file.endswith('.txt'):
            ...
            # 提取关键词和概述
            tokens = content.split()  # 使用split代替word_tokenize，避免nltk.download()
...
clustering = SpectralClustering(n_clusters=200, affinity='nearest_neighbors', n_neighbors=10)
data['Group'] = clustering.fit_predict(X)

# 将结果保存到Excel文件
output_file = os.path.join(folder_path, f'聚类_spectral_{datetime.now().strftime("%Y%m%d%H%M%S")}.xlsx')
