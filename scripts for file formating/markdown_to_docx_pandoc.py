...
def batch_convert_markdown_to_docx(input_directory, output_directory):
    ...
    total_files = sum(file_name.endswith('.md') for _, _, files in os.walk(input_directory) for file_name in files)
    ...
    for root, dirs, files in os.walk(input_directory):
        for file_name in files:
            if file_name.endswith('.md'):
                ...
                output_file = os.path.join(output_directory, os.path.splitext(relative_path)[0] + '.docx')
