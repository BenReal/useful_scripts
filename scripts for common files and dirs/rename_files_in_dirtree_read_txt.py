...
root_path = r"F:\总备份\文献库\S类文献（pdf+txt）\新建文件夹"
...
def check_string_lists(old_names, new_names):
    ...
    # Check if the extensions of the corresponding strings in the two lists are the same
    for i in range(len(old_names)):
        if old_names[i][-4:] != new_names[i][-4:]:
            print(
                f"The strings at index {i} in the two lists have different extensions.")
            return 0
