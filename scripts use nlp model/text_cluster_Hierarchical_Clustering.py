...
X = vectorizer.fit_transform(data['Group']).toarray()  # Convert to dense array

clustering = AgglomerativeClustering(n_clusters=50, linkage='ward')
data['Group'] = clustering.fit_predict(X)

# 将结果保存到Excel文件
output_file = os.path.join(folder_path, f'聚类_hierarchi_{datetime.now().strftime("%Y%m%d%H%M%S")}.xlsx')
